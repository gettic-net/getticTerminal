...
pub fn execute(allocator: std.mem.Allocator) !void {
    try terminal.baslik("Hesap Oluştur");

    var kullanici_adi = try terminal.girdiAl(
        allocator,
        "Kullanıcı adı: ",
    );
    defer allocator.free(kullanici_adi);

    var sifre = try terminal.gizliGirdiAl(
        allocator,
        "Şifre: ",
    );
    ...
    const sonuc = try auth_client.signup(.{
    ...
    try terminal.basari("Hesabınız başarıyla oluşturuldu.");
    try terminal.bilgi("Giriş yapmak için 'gettic login' komutunu kullanın.");
}
